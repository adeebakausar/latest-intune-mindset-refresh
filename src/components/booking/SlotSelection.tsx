...
const timeFilterRanges: Record<TimeFilter, { label: string; icon: React.ReactNode; min: number; max: number }> = {
  ...
  evening: { label: "Evening (5–11)", icon: <Sunset size={14} />, min: 17, max: 23 },
};
...
const SlotSelection = ({ slots, isLoading, selectedTherapist, onTherapistChange, onSlotSelect }: SlotSelectionProps) => {
  ...
  const info = therapistInfo[selectedTherapist];
    ...
    <div className="bg-card rounded-3xl border border-border/50 overflow-hidden shadow-soft">
      ...
      <div className="p-6 pb-0">
        <div className="flex items-center gap-4 p-4 bg-primary/5 rounded-2xl">
          <img src={info.image} alt={info.name} className="w-12 h-12 rounded-full object-cover border-2 border-primary/20" />
          <div>
            <h3 className="font-semibold text-foreground">{info.name}</h3>
            <p className="text-sm text-muted-foreground">{info.title} • {info.price}</p>
          </div>
        </div>
      </div>
